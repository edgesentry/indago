...
class TestPushAisParquet:
    ...
    def test_overwrites_recent_days_even_if_sentinel_already_in_r2(self, tmp_path):
        ...
        three_days_ago_str = (today - timedelta(days=3)).date().isoformat()
        ...
        """)
        ts_today = today - timedelta(minutes=30)
        ts_old = today - timedelta(days=3, hours=1)
        old_date_str = ts_old.date().isoformat()  # derive from actual timestamp, not timedelta arithmetic
        for ts in [ts_today, ts_old]:
            con.execute(
        ...
        mock_fs = MagicMock()
        # R2 already has both dates (empty sentinels from earlier runs)
